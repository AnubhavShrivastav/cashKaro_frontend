    ...
    <>
      ...
      <div className="min-w-[200px] m-7">
        <Slider {...settings}>
          ...
          {HyphenImage.map((image, index) => (
            <div
              key={index}
              className="min-w-[180px] mx-1 relative cursor-pointer"
            >
              <Link key={image.id} to={`/Hyphen/${image.id}`}>

                <img src={image.img} alt={`Image ${index}`} className="" width={410} />

              </Link>
              <img
                src="https://asset20.ckassets.com/resources/image/stores/hyphen-coupon-codes-1735628788.jpg"
